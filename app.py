...
import hashlib, os, requests, json, re, base64
...
# ── GEOCODING (Photon → Nominatim fallback, no API key needed) ───────────────
def smart_geocode(address):
    address = address.strip()
    headers = {'User-Agent': 'DeliverX/3.0 deliverx@app.in'}

    # ── ATTEMPT 1: Photon API (OpenStreetMap based, better for India) ──────────
    # Photon is free, no key, very good at partial/fuzzy addresses
    try:
        r = requests.get('https://photon.komoot.io/api/', params={
            'q': address + ' India',
            'limit': 1,
            'lang': 'en',
            'bbox': '68.1766,8.0767,97.4026,37.0841'  # India bounding box
        ...
        if results.get('features'):
            ...
            for k in ['name','street','district','city','county','state']:
                if props.get(k) and props[k] not in parts:
                    parts.append(props[k])
            display = ', '.join(parts[:3]) if parts else address
            full = ', '.join([p for p in parts if p])
            return {
                'success': True,
                'lat': coords[1], 'lon': coords[0],
                'display': display,
                'full_display': full or address,
    ...
    # ── ATTEMPT 2: Nominatim with smart fallbacks ──────────────────────────────
    parts = [p.strip() for p in re.split(r'[,\n]', address) if p.strip()]
    landmark_match = re.search(r'(?:near|opp|opposite|beside|behind|next to)\s+([^,]+)', address, re.IGNORECASE)
    ...
    stripped = re.sub(r'\b(?:H\.?No\.?|Plot\.?|Flat\.?|#)?\s*\d+[\/\-A-Za-z]*\b', '', address, flags=re.IGNORECASE).strip(' ,')
    if stripped and stripped != address: attempts.append(stripped)
    if landmark and parts: attempts.append(f"{landmark}, {parts[-1]}")
    if len(parts) >= 3: attempts.append(', '.join(parts[-3:]))
    if len(parts) >= 2: attempts.append(', '.join(parts[-2:]))
    if parts: attempts.append(parts[-1])

    seen = set()
    for attempt in attempts:
        attempt = attempt.strip(' ,')
        if not attempt or attempt in seen: continue
        seen.add(attempt)
        try:
            ...
            if results:
                ...
                short_parts = []
                for key in ['road','neighbourhood','suburb','village','town','city','state_district','state']:
    ...
    return {'success': False, 'message': f'Could not locate. Try: "Area Name, City, State"'}
...
def get_route():
    ...
    if not coordinates or len(coordinates) < 2:
        return jsonify({'success': False, 'message': 'Need at least 2 points'})
...
def signup():
    if request.method == 'POST':
        ...
        ref = db.collection('users').add({
            ...
            'created_at': datetime.utcnow().isoformat(), 'lifetime_earnings': 0
        })
...
def get_warehouses():
    ...
    filtered = [w for w in WAREHOUSES if not q or q in w['city'].lower() or q in w['name'].lower() or q in w['state'].lower()]
    return jsonify(filtered[:10])

# ── ADDRESS SUGGESTIONS (live autocomplete) ──────────────────────────────────
...
def suggest():
    ...
    results = []
    try:
        # Photon autocomplete — very fast, India focused
        r = requests.get('https://photon.komoot.io/api/', params={
            'q': q + ' India',
            'limit': 6,
            'lang': 'en',
            'bbox': '68.1766,8.0767,97.4026,37.0841'
        ...
        for feat in data.get('features', []):
            ...
            parts = []
            for k in ['name', 'street', 'district', 'city', 'county', 'state']:
                if p.get(k) and p[k] not in parts:
                    parts.append(p[k])
            if not parts:
                continue
    ...
    if len(results) < 3:
        try:
            ...
            for res in r.json():
                ...
                display = ', '.join(parts[:3]) if parts else res['display_name'].split(',')[0]
                # Avoid duplicates
                if not any(r2['display'] == display for r2 in results):
                    results.append({
                        'display': display,
...
def dashboard():
    ...
    return render_template('dashboard.html', name=session['name'], ride=ride, stops=stops, lifetime=lifetime)
...
def settings():
    ...
    user_doc = db.collection('users').document(session['uid']).get()
    lifetime = user_doc.to_dict().get('lifetime_earnings', 0) if user_doc.exists else 0
    return render_template('settings.html', name=session['name'], lifetime=lifetime, version=APP_VERSION)

@app.route('/records')
def records():
    if 'uid' not in session: return jsonify({'success': False})
    past_rides = db.collection('rides').where('user_id','==',session['uid']).where('status','==','completed').get()
    rides_data = []
    for r in past_rides:
